...
class JsonResp {
  ...
  async marshal(this: JsonResp, res: Response, fieldName: string): Promise<Response> {
    return res
      .status(this.code)
      .json(Object.fromEntries(new Map<string, any>().set("status", this.status).set(fieldName, this.body)));
  }
}

async function NotImplemented(res: Response, usecase: string): Promise<Response> {
  return new JsonResp(Codes.InternalServerError, Bodies.NotImplemented.toString() + `: ${usecase}`).marshal(
    res,
    "error"
  );
}
...
async function InternalServerError(res: Response, body: any): Promise<Response> {
